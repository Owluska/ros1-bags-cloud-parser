...
def corridor_parser(msg, row, **kwargs):
    poly = [[p.x, p.y] for p in msg.points]
    if len(poly) > 3:
        row['corridor'] = Polygon(poly)
    return row
...
def close_objects_parser(msg, row, **kwargs):
    ...
    if not 'dist_thresh' in kwargs or not 'speed_thresh' in kwargs:
        return None
    if row['ego_gear'] != 1 or row['ego_status'] != 1 or row['corridor'] is None:
        return None
    ...
    for state in msg.objects:
        ...
        pos = Point(state.position.x, state.position.y)
        if pos.distance(row['corridor']) < dist_thresh:
...
topics = {
    ...
    '/DEBUG/ego_corridor':
        {'parser': corridor_parser, 'type': 'aux'}
}
